...
**/
public class GWXMapBackedNodeCollection <T extends GWXNode> extends GWXNodeCollection<T> {
	protected final Map<Object, T> nodes;
	
	
	
	public GWXMapBackedNodeCollection (final Map<Object, T> nodes){
		super();
		
		this.nodes = nodes;
	}
	
	public GWXMapBackedNodeCollection (final GWXNode owner, final Map<Object, T> nodes){
		super(owner);
	...
	public void add (final T node){
		...
		ov = nodes.put(GWXUtilities.getNodeId(node), node);
	...
	public void remove (final T node){
		...
		key = GWXUtilities.getNodeId(node);
